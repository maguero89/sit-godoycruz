...
export default function ReportModal({ isOpen, onClose }: ReportModalProps) {
    ...
    return (
        <div className="fixed inset-0 z-[2000] flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm">
            <div className="bg-slate-900 w-full max-w-md max-h-[95vh] flex flex-col rounded-3xl border border-white/10 shadow-2xl animate-in fade-in zoom-in duration-200">
                                ...
                                <form onSubmit={handleSubmit} className="space-y-5">
                                    ...
                                    <div className="flex gap-3 mt-4">
                                        <button
                                            type="button"
                                            onClick={handleClose}
                                            className="w-1/3 bg-slate-700 hover:bg-slate-600 text-white font-bold py-4 rounded-xl transition-all"
                                        >
                                            Cancelar
                                        </button>
                                        <button
                                            type="submit"
                                            disabled={loading}
                                            className="w-2/3 bg-red-600 hover:bg-red-500 disabled:opacity-50 text-white font-bold py-4 rounded-xl flex items-center justify-center gap-3 transition-all"
                                        >
                                            {loading ? "Enviando..." : <><Send size={20} /> Enviar Reporte</>}
                                        </button>
                                    </div>
                                </form>
                            )}
